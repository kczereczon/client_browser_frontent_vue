  ...
  <div class="uk-card uk-margin-top uk-card-default uk-card-body">
    <div class="uk-card-title uk-text-left">{{ title }}</div>
     <vk-grid>
          <div class="uk-width-auto@m">
              <flat-pickr placeholder="Data od" class="uk-input" :config="config" v-model="params.date_from" />
            <!-- <input v-model="params.date_from" class="uk-input" type="text" data-uk-datepicker="{format:'DD.MM.YYYY'}" placeholder="Data od" /> -->
          </div>
          <div class="uk-width-auto@m">
              <flat-pickr placeholder="Data do" class="uk-input" :config="config" v-model="params.date_to" />
            <!-- <input v-model="params.date_to" class="uk-input" type="text" placeholder="Data do" /> -->
          </div>
          <div class="uk-width-auto@m">
            <button @click="loadInvoices()" class="uk-button uk-button-default">SZUKAJ</button>
          </div>
        </vk-grid>
    <vk-table :data="invoices" justified>
      ...
      <vk-table-column
        ...
        ><vk-label slot-scope="{ cell }" :type="cell == 'paid' ? 'success' : 'danger'">{{ cell }}</vk-label>
        </vk-table-column
      >
      <vk-table-column
        class="uk-width-large"
        ...
        cell="invoice_date"
      ><div slot-scope="{cell}">
          {{cell | formatDate}}</div></vk-table-column>
    </vk-table>
  </div>
</template>
  ...
  methods: {
    loadInvoices: async function () {
      ...
      const response = await axios.get(
        "http://127.0.0.1:8000/api/web/invoices/" + this.id,
        { params: params }
      );
